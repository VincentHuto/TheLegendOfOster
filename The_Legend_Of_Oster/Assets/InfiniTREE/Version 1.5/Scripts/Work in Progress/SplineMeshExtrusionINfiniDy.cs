...
public class SplineMeshExtrusionINfiniDy
{
	public class Edge
	{		
	...
	public static void ExtrudeMesh (Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, bool invertFaces)
	{
		...
		ExtrudeMesh(srcMesh, extrudedMesh, extrusion, edges, invertFaces,false,null);
	}	
	
	public static void ExtrudeMesh (Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, Edge[] edges, bool invertFaces, bool cap_to_parent, Vector3[] parent_hole)
	{
			...
			{			
					...
					if(parent_hole!=null){
						if(cap_to_parent & (i == extrusion.Length-1)){
							...
							vertices[v+0] = matrix.MultiplyPoint(inputVertices[e.vertexIndex[0]]*(i*0.2f));
							vertices[v+1] = matrix.MultiplyPoint(inputVertices[e.vertexIndex[1]]*(i*0.2f))*1;
						}
					}else{
							vertices[v+0] = matrix.MultiplyPoint(inputVertices[e.vertexIndex[0]]*(i*0.2f));
							vertices[v+1] = matrix.MultiplyPoint(inputVertices[e.vertexIndex[1]]*(i*0.2f))*1;
