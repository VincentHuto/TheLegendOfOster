...
public class LoomINfiniDy : MonoBehaviour
{
	...
	private static LoomINfiniDy _current;
	private int _count;
	
	public static LoomINfiniDy Current
	{
		get
		{
			Initialize();
	...
	public static void Initialize()
	{
		...
		if (go)
		{
				foreach (var LoomINfiniDy in Resources.FindObjectsOfTypeAll(typeof(LoomINfiniDy)).Cast<LoomINfiniDy>())
					DestroyImmediate(LoomINfiniDy.gameObject);
				var g = new GameObject("LoomINfiniDy");
				_current = g.AddComponent<LoomINfiniDy>();
			_initialized = true;
