...
Shader "InfiniGRASS/InfiniTREE_Wind_Circular" {
    Properties {
        ...
        _OceanCenter("Ocean Center", Vector) = (0, 0, 0, 0)
         ...
         _RippleScale ("Vary Height", Float ) = 0     
		   ...
		   //v2.0.7
		   _InteractPosC1("Circular Wind Pos 1", Vector) = (0, 0, 0, 0)
		   _InteractPosC2("Circular Wind Pos 2", Vector) = (0, 0, 0, 0)
		   _InteractAmpFreqRad1("Circular Wind 1 Amp Freq Rad", Vector) = (0, 0, 0, 0)
		   _InteractAmpFreqRad2("Circular Wind 2 Amp Freq Rad", Vector) = (0, 0, 0, 0)
    }
    SubShader {
        Tags {
            "Queue"="AlphaTest"
        ...
        Pass {
           ...
           // #pragma exclude_renderers gles xbox360 ps3 flash 
			...
			//v2.0.7
			float4 _InteractPosC1;
			float4 _InteractPosC2;
			float4 _InteractAmpFreqRad1;
			float4 _InteractAmpFreqRad2;
  ...
  //
				...
				///////////////// TORNADOS - CIRCULAR WINDS
				bool enterTornadoB = false;
				/////////////////////////////////////////////////////////////////////////////////////////// LOCAL INTERACTOR 1 /////////////////////////////////////////////
				if (_InteractAmpFreqRad2.x != 0 && distance(_InteractPosC2, o.posWorld) < _InteractAmpFreqRad2.z * 1) {
					distA = distance(_InteractPosC2, o.posWorld) / (_InteractAmpFreqRad2.z * 1);					
					//float2 SpeedFac2 = 3 * (o.posWorld - _InteractPosC2) * 1 //_WaveControl1.w
					//	+ _InteractAmpFreqRad2.z*(0.1*cross(float3(0, 1, 0.5), o.posWorld - _InteractPosC2) - 0.71*cross(float3(0.2, 1, 0), _InteractPosC2 - o.posWorld))
					//	*_InteractAmpFreqRad2.x*sin(o.posWorld.z + _InteractAmpFreqRad2.y*_Time.y)*(o.posWorld - _InteractPosC2) * 2;
					float3 SpeedFac2 = cross(float3(0, 1, 0.0), o.posWorld - _InteractPosC2)*_InteractAmpFreqRad2.x *abs(0.1*sin(1/distA + _InteractAmpFreqRad2.y*_Time.y)+0.2);
					//*_InteractAmpFreqRad2.x*sin(o.posWorld.z + _InteractAmpFreqRad2.y*_Time.y)*(o.posWorld - _InteractPosC2) * 2;
					//+ _InteractAmpFreqRad2.x*(o.posWorld - _InteractPosC2) * 2 * sin(o.posWorld.z + _InteractAmpFreqRad2.y*_Time.y);
					_BulgeScale = _BulgeScale * (1 / (distA + 0.01));
					if (o.uv0.y > 0.19) {
						_WaveXFactor = (1 - distA)*(1 - distA)*SpeedFac2.z;
						_WaveYFactor = (1 - distA)*(1 - distA)*SpeedFac2.x;
					}
					if (o.uv0.y > 0.5) {
						o.posWorld.y = o.posWorld.y - (1 - distA) *2 * _InteractAmpFreqRad2.w * (o.uv0.y - 0.5)*sin(o.posWorld.z + _Time.y);//+_BulgeScale*0.5*cos(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*sin(o.posWorld.z+_Time.y) ;
					}
					enterTornadoB = true;
					if (o.uv0.y > 0.25) {
						o.posWorld.x += (_WaveYFactor * 1 );
						o.posWorld.z += (_WaveXFactor * 1 );
					}
				}
				/////////////////////////////////////////////////////////////////////////////////////////// END LOCAL INTERACTOR 1 /////////////////////////////////////////////
				/////////////////////////////////////////////////////////////////////////////////////////// LOCAL INTERACTOR 2 /////////////////////////////////////////////				
				bool enterTornadoA = false;
				if (_InteractAmpFreqRad1.x != 0 && distance(_InteractPosC1, o.posWorld) < _InteractAmpFreqRad1.z * 1) {
					//SpeedFac = float3(0, 0, 0);
					distA = distance(_InteractPosC1, o.posWorld) / (_InteractAmpFreqRad1.z * 1);
					float3 SpeedFac2 = cross(float3(0, 1, 0.0), o.posWorld - _InteractPosC1)*_InteractAmpFreqRad1.x *abs(0.1*sin(1 / distA + _InteractAmpFreqRad1.y*_Time.y) + 0.2);
					/*SpeedFac += 3 * (o.posWorld - _InteractPosC1) *1
						+ _InteractAmpFreqRad1.z*(1.1*cross(float3(0, 1, 0.5), o.posWorld - _InteractPosC1) - 2.71*cross(float3(0.2, 1, 0), _InteractPosC1 - o.posWorld))
						*_InteractAmpFreqRad1.x*sin(o.posWorld.z + _InteractAmpFreqRad1.y*_Time.y)*(o.posWorld - _InteractPosC1) * 2;*/
						//+ _InteractAmpFreqRad1.x*(o.posWorld - _InteractPosC1) *_WaveControl1.w *sin(o.posWorld.z + _InteractAmpFreqRad1.y*_Time.y);
					_BulgeScale = _BulgeScale * (1 / (distA + 0.01));
					if (o.uv0.y > 0.19) {
						_WaveXFactor = (1 - distA)*(1 - distA)*SpeedFac2.z;
						_WaveYFactor = (1 - distA)*(1 - distA)*SpeedFac2.x;
					...
					if (o.uv0.y > 0.5) {
						o.posWorld.y = o.posWorld.y - (1 - distA) * 2 * _InteractAmpFreqRad1.w * (o.uv0.y - 0.5)*sin(o.posWorld.z + _Time.y);
					}
					enterTornadoA = true;
					if (o.uv0.y > 0.25) {
						o.posWorld.x += (_WaveYFactor * 1);
						o.posWorld.z += (_WaveXFactor * 1);
						/*o.posWorld.x += (_WaveYFactor * 0.001 + _WaveXFactor * 0.001) / distA;
						o.posWorld.z += (_WaveXFactor * 0.001 + _WaveYFactor * 0.001) / distA;*/
					}
				}
				/////////////////////////////////////////////////////////////////////////////////////////// END LOCAL INTERACTOR 2 /////////////////////////////////////////////
				
				///////////////// END TORNADOS - CIRCULAR WINDS


				distA = distance(_InteractPos, o.posWorld) / (_StopMotionThreshold * 1);
                  if( distance(_InteractPos,o.posWorld) < _StopMotionThreshold*1   && !enterTornadoB && !enterTornadoA){
...
//						}
	       ...
	       //         	v.vertex.y = v.vertex.y *_RandYScale* abs(cos(((_TimeControl1.w*_Time.y +_TimeControl1.z)*0.2 + 2*dist)*_RippleScale));
	          ...
	          // _OceanCenter.z = 0.0;
				...
				///////////////////////// 
						 if (!enterTornadoB && !enterTornadoA) {
							 if (o.uv0.y > 0.1) {
