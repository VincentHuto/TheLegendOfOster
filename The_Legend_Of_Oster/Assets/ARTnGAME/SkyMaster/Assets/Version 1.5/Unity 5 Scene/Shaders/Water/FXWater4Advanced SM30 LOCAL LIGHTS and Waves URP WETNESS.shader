...
Shader "SkyMaster/Water4 SM3.0 Local Lights - Waves URP WETNESS" {
Properties {
...
CGINCLUDE
	...
	sampler2D _MyGrabTexture;//v4.8.9 LWRP
	float4 _MyGrabTexture_TexelSize;
	sampler2D _RefractionTextureSM;//v4.8.9 LWRP
	float4 _RefractionTextureSM_TexelSize;
	//_PlanarReflectionTexture
	sampler2D _PlanarReflectionTexture;//v4.8.9 LWRP
	...
	half4 frag600( v2f i ) : SV_Target
	{
		...
		half4 rtRefractionsNoDistort = tex2Dproj(_RefractionTextureSM, UNITY_PROJ_COORD(i.grabPassPos));
		...
		half4 rtRefractions = tex2Dproj(_RefractionTextureSM, UNITY_PROJ_COORD(grabWithOffset));
		
		//#ifdef WATER_REFLECTIVE
			half4 rtReflections = tex2Dproj(_PlanarReflectionTexture, UNITY_PROJ_COORD(screenWithOffset));
		//#endif
...
//		float4 lighter = float4(_LightColor0.rgb*atten*2*diff*diff*1*diff*(1-refl2Refr)*1,1);
		...
		return baseColor;// float4(1, 1, 1, 0.4);// baseColor * 1;
	}
...
Subshader
{
	...
	//Tags {"LightMode"="ForwardBase"}
		Tags{ "LightMode" = "MyCustomPass" } //v4.8.9 LWRP
