...
public class CameraShakeManager : MonoBehaviour {
	...
	/// </summary>
	private List<CameraShakeA> m_activeShakes = new List<CameraShakeA>();
	...
	void LateUpdate() {
		...
		foreach (var shake in m_activeShakes.Reverse<CameraShakeA>()) {
	...
	public CameraShakeA Play(string name) {
        var cs = Instantiate(shakeOBJ, transform);//  Instantiate(Resources.Load<GameObject>(name), transform) as GameObject;
		if (cs != null) {
			m_activeShakes.Add(cs.GetComponent<CameraShakeA>());
		}
		return cs.GetComponent<CameraShakeA>();
	}
	...
	public void Stop(CameraShakeA shake, bool immediate = false) {
