...
namespace Artngame.GIPROXY {
	...
	void Start () {
			...
			Initial_camera_pos = Camera.main.transform.position;
		...
		void OnGUI(){
			...
			if(intro_ended){
				...
				if(GUI.Button(new Rect(0,100,110,30),"Toggle GUI "+GUI_ON)){
			...
			if(GI_PROXY_script!=null){
					...
					if(GUI.Button(new Rect(200,10+19-20,90+10,20),"Reset camera")){
						Camera.main.transform.position = Initial_camera_pos;
					}

				string GI_ON = "Toggle GI on";
				if(GI_PROXY_script.enabled){GI_ON = "Toggle GI off - "+GI_PROXY_script.Lights.Count+" Lights";}

				if(GUI.Button(new Rect(200,10+19,200+10,80-19),GI_ON)){
				...
				if(GI_PROXY_POINT_script.enabled){GI_ON_POINT = "Toggle Point GI off - "+GI_PROXY_POINT_script.Lights.Count+" Lights";}
					...
					if(GUI.Button(new Rect(400+10,50,200-10,40),GI_ON_POINT)){
...
//				this.gameObject.transform.root.eulerAngles =new Vector3(rotation_current_X,this.gameObject.transform.root.eulerAngles.y,this.gameObject.transform.root.eulerAngles.z);
				...
				position_current_Z = GUI.HorizontalSlider(new Rect(0,400,200,30),PointLight.transform.position.z,-45+35,-2.5f+55);
				...
				if(PointLight!=null){
					...
					if(1==1){
						if(GUI.Button(new Rect(400+10,10+19,200-10,40-19),"Toggle Point Light "+ONOFF) & 1==1){
				...
				if(SpotLight!=null){
					...
					if(1==1){
						if(GUI.Button(new Rect(0,10+19,200,40-19),"Toggle Spot Light "+ONOFFSPOT)){
				...
				if(PointLight!=null){
						if(GUI.Button(new Rect(400+10,100,200-10,40),"Toggle Point GI power "+HIGH_POINT) & 1==1){
